...
st.caption("提携業者間でのリアルタイム需給調整システム")

# --- 1. 自社の状況を更新する（供給側） ---
with st.expander("➕ 自社の空き状況を登録・更新する"):
    with st.form("update_form"):
        company_name = st.selectbox("自社名を選択", ["警備A社", "警備B社", "警備C社", "警備D社"])
        available_count = st.number_input("応援に出せる人数", min_value=0, step=1)
...
# 人数が1人以上の会社のみ表示
available_list = st.session_state.fleet_data[st.session_state.fleet_data["空き人数"] > 0]

if not available_list.empty:
    st.table(available_list)
    
    # 仮予約機能
    target_company = st.selectbox("応援を依頼したい会社を選択", available_list["会社名"])
    request_num = st.number_input("依頼したい人数", min_value=1, step=1)
    
    if st.button("仮予約（プッシュ通知送信）"):
        # ここで本来はLINEやメールで通知を飛ばす
        st.warning(f"【仮予約実行】{target_company}へ{request_num}名の応援依頼を送りました。相手方の承諾を待ってください。")
else:
    st.info("現在、応援可能な会社はありません。")
